...
static const char *adsp_binary_name[] = {
	"",
	"hifi4dsp_load_11.bin", "hifi4dsp_load_12.bin", "hifi4dsp_load_13.bin", "hifi4dsp_load_14.bin",
	"hifi4dsp_load_21.bin", "hifi4dsp_load_22.bin", "hifi4dsp_load_23.bin", "hifi4dsp_load_24.bin"
};
			...
			phys_addr_t dram_base)
{
	...
	if (err) {
		pr_err("firmware may be corrupted!\n");
		goto ERROR;
	}
			...
			void *context)
{
	...
	if (!fw) {
		...
		pr_err("request_firmware_nowait not available.\n");
		goto TAIL;
	}

	pr_info("firmware load success, size = %d\n", (int)fw->size);
...
int async_load_hifixdsp_bin_and_run(
			callback_fn callback, void *param, int model)
{
	int ret = 0;
	struct adsp_chip_info *adsp;
