...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
	...
	if ( 1 == data->pdata->swap_x2y )
	{
		swap_point = events[i].x;
		events[i].x = data->pdata->x_max- events[i].y;
		events[i].y = swap_point;
	}
				...
				struct fts_ts_platform_data *pdata)
{
	...
	if ( 1 == pdata->swap_x2y )
	{
		pdata->x_max = coords[3];
		pdata->y_max = coords[2];
	}else{
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "focaltech,swap-x2y", &pdata->swap_x2y);
    if (ret < 0)
		FTS_ERROR("swap x2y enabled!");
...
int fts_i2c_ping_dev(struct fts_ts_data *ts_data)
{
	int ret = 0;
	u8 chip_id[2] = { 0 };
	...
	ret = fts_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
	if (ret < 0) {
		FTS_INFO("fw is invalid or no device, need read boot id");
		if (ts_data->ic_info.hid_supported) {
			fts_hid2std();
		}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ret = fts_i2c_ping_dev(ts_data);
    if (ret) {
	FTS_ERROR("No ft touch device found.");
	goto err_irq_req;
    }
...
static int fts_ts_suspend(struct device *dev)
{
	...
	FTS_DYNAMIC("Already in suspend state");
	return 0;
    }
	...
	FTS_DYNAMIC("Enter into gesture mode(suspend)");
	...
	FTS_DYNAMIC("set TP to sleep mode ok");
    }

    if (!ts_data->ic_info.is_incell) {
	FTS_DYNAMIC("TP module isn't incell");
#if FTS_POWER_SOURCE_CUST_EN
	...
	FTS_DYNAMIC("TP module is incell");
    }
...
static int fts_ts_resume(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
    if (!ts_data->suspended) {
	FTS_ERROR("Already in awake state");
	return 0;
    }
