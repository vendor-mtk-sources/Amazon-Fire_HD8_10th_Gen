...
#include "gc2375hmipi_cxt_front_Sensor.h"

/**************** Modify Following Strings for Debug ****************/
#define PFX "gc2375h_cxt_front_camera_sensor"
...
kal_bool GC2375H_CXT_FRONT_DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375HMIPI_CXT_FRONT_SENSOR_ID,
...
static void sensor_reg_dump(void)
{
	...
	LOG_INF("+");
	LOG_INF("Camera_sensor RegDump start");

	write_cmos_sensor(0xfe, 0x00);

	LOG_INF("shutter time[5:0] -- 0x03 -- 0x%x\n", read_cmos_sensor(0x03));
	LOG_INF("shutter time[7:0] -- 0x04 -- 0x%x\n", read_cmos_sensor(0x04));
	LOG_INF("shutter time = 0x%x\n", (read_cmos_sensor(0x03) << 8) | read_cmos_sensor(0x04));

	LOG_INF("frame length[5:0] -- 0x07 -- 0x%x\n", read_cmos_sensor(0x07));
	LOG_INF("frame length[7:0] -- 0x08 -- 0x%x\n", read_cmos_sensor(0x08));
	LOG_INF("frame length = 0x%x\n", (read_cmos_sensor(0x07) << 8) + read_cmos_sensor(0x08) + 1224);

	LOG_INF("line length[5:0] -- 0x05 -- 0x%x\n", read_cmos_sensor(0x07));
	LOG_INF("line length[7:0] -- 0x06 -- 0x%x\n", read_cmos_sensor(0x08));
	LOG_INF("line length = 0x%x\n", (read_cmos_sensor(0x05) << 8) + read_cmos_sensor(0x06) + 438);

	LOG_INF("stream on[7:0] -- 0xef -- 0x%x\n", read_cmos_sensor(0xef));
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	GC2375H_CXT_FRONT_DuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF
			("current_fps %d not support, cap's setting: %d fps!\n",
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				LOG_INF
				("cur_fps %d error, cap's setting: %d fps!\n",
				framerate,
				imgsensor_info.cap.max_framerate / 10);
			frame_length =
...
UINT32 GC2375HMIPI_CXT_FRONT_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
					**pfFunc)
{
