...
#define GET_ADSP_MODEL() (g_priv->dsp_model)
	...
	struct mt8168_adsp_debugfs_data *dump)
{
	...
	while (copy > 0) {
		...
		if (from > 0) {
#ifdef MT8168_AUDIO_SPI_DUMP_DMA_DEBUG
...
static void do_adsp_dump_release(struct mt8168_adsp_debugfs_data *dump)
{
	...
	if (dump->enable) {
		...
		param.task_scene_id = dump->task_scene;
	...
	//mt8168_adsp_clear_power_state(dump->scene);
	...
	struct file *file)
{
	...
	//mt8168_adsp_notify_power_state(dump->scene);
	...
	dump->ack_size = ack_param.request_bytes;
	dump->buffer_size = ack_param.period_cnt * ack_param.period_size_bytes;
	...
	cpu_dma->period_size_bytes = ack_param.period_size_bytes;
...
static const struct mt8168_adsp_debugfs_attr adsp_debugfs_attrs[] = {
	{"adsp_dump_ffpin", &mt8168_adsp_dump_fops, PRE_PROC_IN_DUMP},
	{"adsp_dump_ffpout", &mt8168_adsp_dump_fops, PRE_PROC_OUT_DUMP},
};
...
static void mt8168_adsp_init_debug_dump(struct mt8168_adsp_pcm_priv *priv)
{
	...
	for (i = 0; i < debugfs_count; i++) {
		...
		dump_data->task_scene = TASK_SCENE_VA;
...
static int mt8168_adsp_dai_suspend(struct snd_soc_dai *dai)
{
	...
	cancel_delayed_work_sync(&g_priv->boost_work);
	...
	hostless = (mt8168_adsp_hostless_active() > 0);
	if (hostless) {
...
static const char *adsp_model[] = {
	"Stop", "Alexa_enUS", "Alexa_enGB", "Alexa_deDE", "Alexa_jaJP", "Amazon_enUS", "Amazon_enGB", "Amazon_deDE", "Amazon_jaJP"
};
static const SOC_ENUM_SINGLE_DECL(mt8168_adsp_model_enum, 0, 0, adsp_model);

static int mt8168_adsp_model_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = GET_ADSP_MODEL();
	return 0;
}

static int mt8168_adsp_model_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	int ret = 0;
	int model = ucontrol->value.enumerated.item[0];

	if (model == GET_ADSP_MODEL())
		return 0;
	if ((model < 0) || (model >=  sizeof (adsp_model) / sizeof(adsp_model[0])))
		return -EINVAL;
	...
	if (model != 0) {
		/* If there's another DSP FW running in progress, we have to stop it first */
		if (GET_ADSP_MODEL() != 0) {
		...
		/* load the new DSP FW and start */
		priv->dsp_loading = true;
		ret =
		async_load_hifixdsp_bin_and_run(load_hifi4dsp_callback, priv, model);
		if (ret) {
			dev_dbg(plat->dev,
...
static const struct snd_kcontrol_new mt8168_adsp_controls[] = {
	SOC_ENUM_EXT("ADSP Model",
			    mt8168_adsp_model_enum,
			    mt8168_adsp_model_get,
			    mt8168_adsp_model_put),
	...
	struct mt8168_adsp_dai_memory *dai_mem)
{
	...
	copy_bytes = (avail_bytes / adsp_period_size_bytes) * adsp_period_size_bytes;
...
static void disable_boost_work(struct work_struct *work)
{
	...
	ret = update_userlimit_cpu_freq(CPU_KIR_DSP, clstr_num, &pld);
	if (ret != 0)
		pr_err("%s disable CPU boost fail, ret = %d\n", __func__, ret);

}

static void set_dynamic_boost_boost(int duration)
{
	...
	pld.max = curr_policy.cpuinfo.max_freq;
	pld.min = curr_policy.cpuinfo.max_freq;
	clstr_num = (unsigned int)arch_get_nr_clusters();
	if (!mod_delayed_work(system_wq,
			&g_priv->boost_work, msecs_to_jiffies(duration))) {
		ret = update_userlimit_cpu_freq(CPU_KIR_DSP, clstr_num, &pld);
		if (ret != 0)
			pr_err("%s enable CPU boost fail, ret = %d\n", __func__, ret);
	}
}
...
static void mt8168_adsp_pcm_va_notify(struct dsp_ipc_va_notify *info)
{
	...
	if (info->type == VA_NOTIFY_VAD_PASS)
		...
		set_dynamic_boost_boost(250);
		__pm_wakeup_event(&g_priv->vad_wake, 800);
#ifdef CONFIG_AMAZON_METRICS_LOG
...
static int mt8168_adsp_pcm_probe(struct snd_soc_platform *platform)
{
	...
	priv->dsp_loading = true;
	ret = async_load_hifixdsp_bin_and_run(load_hifi4dsp_callback, priv, 1);
	if (ret) {
		dev_err(platform->dev,
			"%s async_load_hifi4dsp_bin_and_run fail %d\n",
			__func__, ret);
...
static int mt8168_adsp_pcm_dev_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&g_priv->boost_work, disable_boost_work);
	wakeup_source_init(&g_priv->vad_wake, "adsp_wake");
