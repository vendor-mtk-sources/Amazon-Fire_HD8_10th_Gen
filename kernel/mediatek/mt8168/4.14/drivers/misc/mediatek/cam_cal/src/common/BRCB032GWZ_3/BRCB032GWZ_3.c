 ...
 *	I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0xAA >> 1)};
 */
...
static int iWriteCAM_CAL(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	...
	if (a_u4Bytes + 2 > 8) {
		CAM_CALDB(
		"[BRCB032GWZ] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
...
static int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	...
	if (ui4_length > 8) {
		CAM_CALDB(
		  "[BRCB032GWZ] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
#if 0
static int iWriteData(unsigned int ui4_offset,
		unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	CAM_CALDB("[BRCB032GWZ] %s u4CurrentOffset is %d\n",
			__func__,
			u4CurrentOffset);

	do {
		if (i4ResidueDataLength >= 6) {
			i4RetValue = iWriteCAM_CAL(
				(u16)u4CurrentOffset, 6, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB(
				    "[CAM_CAL] I2C write data failed!!\n");
				return -1;
			}
			u4IncOffset += 6;
	...
	CAM_CALDB("[BRCB032GWZ] I2C write data done\n");
...
static int iReadData(unsigned int  ui4_offset,
	unsigned int  ui4_length, unsigned char *pinputdata)
{
	int  i4RetValue = 0;
	int  i4ResidueDataLength;
	...
	do {
		if (i4ResidueDataLength >= 8) {
			i4RetValue = iReadCAM_CAL(
				(u16)u4CurrentOffset, 8, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB(
				    "[BRCB032GWZ] I2C read data failed!!\n");
				return -1;
			}
			u4IncOffset += 8;
...
unsigned int brcb032gwz_selective_read_region(
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				CAM_CALDB(
				  "[BRCB032GWZ] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	CAM_CALDB(
	  "[BRCB032GWZ] init Working buffer address 0x%8x  command is 0x%8x\n",
	  (u32)pWorkingBuff, (u32)a_u4Command);


	if (copy_from_user((u8 *)pWorkingBuff,
	...
	case CAM_CALIOC_G_READ:
		...
		CAM_CALDB(
		  "[BRCB032GWZ] Before read Working buffer address 0x%8x\n",
		  (u32)pWorkingBuff);

		/* i4RetValue = iReadData((u16)ptempbuf->u4Offset,
		 *			ptempbuf->u4Length, pWorkingBuff);
		 */
		CAM_CALDB(
		"[BRCB032GWZ_3_CAM_CAL] After read Working buffer data 0x%4x\n",
		*pWorkingBuff);
		...
		if (ptempbuf->u4Offset == 0x800) {
			...
			*(u32 *)pWorkingBuff = 0x100CB032;
			...
			puSendCmd[0] = 0x7E;
			/* iReadRegI2C(puSendCmd , 1,
			 *	pWorkingBuff,2, (0x53<<1) );
			 */
			...
			while (readTryagain > 0) {
				i4RetValue = iReadData(
					(u16)ptempbuf->u4Offset,
					ptempbuf->u4Length,
					pWorkingBuff);

				CAM_CALDB(
				  "[BRCB032GWZ] error (%d) Read retry (%d)\n",
		...
		if (1) {
			...
			CAM_CALDB(
			  "[BRCB032GWZ]enter EEPROM_test function%d (%x)%x\n",
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[BRCB032GWZ] to user length %d\n",
			ptempbuf->u4Length);
		CAM_CALDB(
			"[BRCB032GWZ] to user  Working buffer address 0x%8x\n",
			(u32)pWorkingBuff);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params,
			(u8 *)pWorkingBuff, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
...
static int CAM_CAL_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	CAM_CALDB("[BRCB032GWZ_3_CAM_CAL] %s\n", __func__);
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (g_pCAM_CAL_CharDrv == NULL) {
		...
		CAM_CALDB(
		    "[BRCB032GWZ_3_CAM_CAL] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB(
		    "[BRCB032GWZ_3_CAM_CAL] Attatch file operation failed\n");
	...
	CAM_CAL_device = device_create(CAM_CAL_class, NULL,
				g_CAM_CALdevno, NULL, CAM_CAL_DRVNAME);
...
static int CAM_CAL_i2c_detect(struct i2c_client *client,
...
#else
#endif
static int CAM_CAL_i2c_probe(struct i2c_client *client,
...
#ifndef CAM_CAL_ICS_REVISION
static int CAM_CAL_i2c_detect(
	struct i2c_client *client, int kind, struct i2c_board_info *info)
{
	strcpy(info->type, CAM_CAL_DRVNAME);
	return 0;
}
#endif
static int CAM_CAL_i2c_probe(
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (i4RetValue) {
		CAM_CALDB(
		  "[BRCB032GWZ_3_CAM_CAL] register char device failed!\n");
		return i4RetValue;
	}
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB(
		  "failed to register BRCB032GWZ_3_CAM_CAL driver, 2nd time\n");
		return -ENODEV;
	}
