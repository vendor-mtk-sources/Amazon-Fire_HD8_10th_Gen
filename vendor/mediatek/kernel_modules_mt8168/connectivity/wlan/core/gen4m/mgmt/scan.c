				  ...
				  IN struct SW_RFB *prSwRfb)
{
	...
	if (prCountryIE && prCountryIE->ucLength ==
			(sizeof(struct IE_COUNTRY) - ELEM_HDR_LEN)) {
	...
	IN struct ADAPTER *prAdapter, IN uint8_t ucBssIndex)
{
	...
	log_dbg(SCN, INFO, "SEARCH: Bss Num: %d, Look for SSID: xxxx, "
...
void scanParseVHTCapIE(IN uint8_t *pucIE, IN struct BSS_DESC *prBssDesc)
{
	...
	/* Error handling */
	if (IE_LEN(prVhtCap) != (sizeof(struct IE_VHT_CAP) - ELEM_HDR_LEN)) {
		DBGLOG(SCN, WARN,
