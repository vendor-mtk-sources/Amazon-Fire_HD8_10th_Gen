...
static int rt551x_dsp_state_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = rt551x_state;
	return 0;
}

static int rt551x_dsp_state_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
static const struct snd_kcontrol_new rt551x_snd_controls[] = {
	...
	SOC_SINGLE_EXT("DSP Exist", SND_SOC_NOPM, 0, 1, 0,
		rt551x_dsp_state_get, rt551x_dsp_state_put),
};
...
static int rt551x_suspend(struct snd_soc_codec *codec)
{
	...
	pr_info("%s -- OK!\n", __func__);
	cancel_delayed_work_sync(&rt551x->boost_work);
...
static void set_dynamic_boost_boost(int duration, struct rt551x_priv *rt551x)
{
	...
	if (!mod_delayed_work(system_wq,
			&rt551x->boost_work, msecs_to_jiffies(duration))) {
...
static void rt551x_handler_work(struct work_struct *work)
{
	...
	set_dynamic_boost_boost(250, rt551x);
		    ...
		    const struct i2c_device_id *id)
{
	...
	default:
		...
		rt551x->fp_readable_register = rt5518_readable_register;
		ret = rt5518_hw_reset_gpio_init(rt551x);
		// "rt551x_state = 1" means using rt551x voice DSP.
		// Since very few of RT5518 has problem, see MUSPLAT-3885
		// somtimes even has external RT5518 DSP, it can go here.
		// we need to remove it, only support RT5518 DSP.
		// rt551x_state = 0;
	}
	...
	INIT_DELAYED_WORK(&rt551x->boost_work, disable_boost_work);
