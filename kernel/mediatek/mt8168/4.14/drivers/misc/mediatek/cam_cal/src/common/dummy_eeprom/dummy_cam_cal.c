		...
		unsigned char *pinputdata)
{
	...
	if ((ui4_offset >= 0x2000) || (ui4_length >= 0x2000) || (ui4_offset + ui4_length >= 0x2000)) {
		CAM_CALDB(
		"[CAM_CAL] Write Error!! S-24CS64A not supprt address >= 0x2000!!, offset:%u, length:%u\n", ui4_offset, ui4_length);
		return -1;
	}
	...
	pBuff = pinputdata;
	CAM_CALDB("[CAM_CAL] %s u4CurrentOffset is %d\n",
		__fucn__, u4CurrentOffset);
	do {
		if (i4ResidueDataLength >= 6) {
			...
			i4RetValue = iWriteCAM_CAL((u16)u4CurrentOffset,
	...
	CAM_CALDB("[CAM_CAL] I2C write data done\n");
	...
	unsigned char *pinputdata)
{
	...
	if ((ui4_offset >= 0x2000) || (ui4_length >= 0x2000) || (ui4_offset + ui4_length >= 0x2000)) {
		CAM_CALDB(
		"[CAM_CAL] Read Error!! S-24CS64A not supprt address >= 0x2000!!, offset:%u, length:%u\n", ui4_offset, ui4_length);
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			...
			i4RetValue = iReadCAM_CAL((u16)u4CurrentOffset,
				i4ResidueDataLength, pBuff);
			if (i4RetValue != 0) {
				CAM_CALDB("[CAM_CAL] I2C read data failed!!\n");
				return -1;
	...
	CAM_CALDB("[CAM_CAL] I2C read data done\n");
	return 0;
}
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				CAM_CALDB(
				"[CAM_CAL] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
...
static unsigned short force[] = {CAM_CAL_I2C_GROUP_ID,
				 S24CS64A_DEVICE_ID,
				 I2C_CLIENT_END,
				 I2C_CLIENT_END};
#else
