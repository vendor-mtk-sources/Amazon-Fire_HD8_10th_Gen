...
static struct iio_channel *g_vbus_iio_channel;
static int wpc_present;
static bool vbus_auxadc_flag;

/*
 * @Function       vbus_get_auxadc_data
 * @Description    Reads ADC for USB voltage
 * @Input          none
 * @Return         char          Voltage in mV
 */
static int vbus_get_auxadc_data(void)
{
	int val;
	int ret;
	int volt;

	ret = iio_read_channel_processed(g_vbus_iio_channel, &val);
	if (ret < 0) {
		pr_notice("Integrity IIO channel read FAIL\n");
		return -1;
	}
	volt = (int)((val * ONYX_VBUS_SCALE_FACTOR) / (1000));
	return volt;
}
...
static int get_battery_property(struct integrity_driver_data *data, int type)
{
	...
	case TYPE_VBAT:
		psy = data->batt_psy;
		property = POWER_SUPPLY_PROP_VOLTAGE_NOW;
		break;
	case TYPE_TBAT:
		psy = data->batt_psy;
	...
	case TYPE_BAT_CYCLE:
		psy = data->batt_psy;
		property = POWER_SUPPLY_PROP_CYCLE_COUNT;
		break;
	case TYPE_CHG_STATE:
		psy = data->batt_psy;
...
static void integrity_module_routine(struct work_struct *work)
{
	...
	if (vbus_auxadc_flag && wpc_present != 1)
		vbus = vbus_get_auxadc_data();
	else
		vbus = get_battery_property(data, TYPE_VBUS) / 1000;
	vbat = get_battery_property(data, TYPE_VBAT) / 1000;
	...
	if (soc < 0 || vbus < 0 || vbat < 0 || cycle < 0 || chg_sts < 0 || chg_type < 0)
		goto skip;
	if (!(chg_type >= CHARGER_UNKNOWN && chg_type <= WIRELESS_10W_CHARGER))
		goto skip;
	...
	pr_debug("integrity: soc[%d] vbus[%d] vbat[%d] tbat[%d] cycle[%d] chg_sts[%d] chg_type[%d] t_v[%d] elaps[%lu]\n",
...
static int integrity_module_probe(struct platform_device *pdev)
{
	...
	vbus_auxadc_flag = of_property_read_bool(pdev->dev.of_node, "vbus_auxadc_present");
	if (vbus_auxadc_flag) {
		...
		if (chg_dev) {
			...
			wpc_present = 0;
			g_vbus_iio_channel = iio_channel_get(&pdev->dev, "vbus-channel");
			if (IS_ERR(g_vbus_iio_channel)) {
				ret = -EPROBE_DEFER;
				goto out;
			}
		}
