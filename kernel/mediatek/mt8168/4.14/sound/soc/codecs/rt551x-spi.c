...
static int rt551x_spi_prepare(struct snd_pcm_substream *substream)
{
	...
	if (rt551x_dsp->buf_base == 0) {
		...
		if (!rt551x_dsp->pcm_is_readable) {
			...
			mutex_unlock(&rt551x_dsp->dma_lock);
			/*We use msleep to stop driver & alsa print to much fail log which may cause watch dog*/
			/*Only abnormal condition will cause this situation, eg: device have no external dsp, dsp crash, dsp hardware damage */
			/*Some onyx device have no external dsp will also cause this situation*/
			msleep(500);
			return -1;
		}
		rt551x_dsp->dma_offset = 0;
