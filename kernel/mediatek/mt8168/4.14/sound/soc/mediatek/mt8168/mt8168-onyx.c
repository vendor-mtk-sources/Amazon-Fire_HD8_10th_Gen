/*
 * mt8168-onyx.c  --  MT8168 machine driver
 *
...
static const char * const mt8168_onyx_pin_str[PIN_STATE_MAX] = {
	"aud_mosi_on",
	"aud_mosi_off",
	"aud_miso_on",
	"aud_miso_off",
...
struct mt8168_onyx_priv {
	...
	struct mt8168_onyx_dmic_ctrl_data dmic_data;
	struct mt8168_onyx_tdm_ctrl_data tdm_data;
};
...
const char * const EXT_SPK_PGA_GAIN[] = {"11dB", "14dB", "19dB", "25dB"};

static const struct soc_enum mt8168_onyx_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(EXT_SPK_PGA_GAIN), EXT_SPK_PGA_GAIN),
};

static int mt8168_onyx_ext_speaker_pga_get(struct snd_kcontrol *kcontrol,
...
static int mt8168_onyx_ext_speaker_pga_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_card *card = snd_kcontrol_chip(kcontrol);
	struct mt8168_onyx_priv *card_data =
...
static const struct snd_kcontrol_new mt8168_onyx_controls[] = {
	SOC_ENUM_EXT("Ext_Speaker_PGA_Gain",
		mt8168_onyx_enum[0],
		mt8168_onyx_ext_speaker_pga_get,
		mt8168_onyx_ext_speaker_pga_put),
};
...
static void mt8168_onyx_ext_hp_amp_spk_turn_on(struct snd_soc_card *card)
{
	struct mt8168_onyx_priv *card_data =
		snd_soc_card_get_drvdata(card);
...
static void mt8168_onyx_ext_hp_amp_spk_turn_off(struct snd_soc_card *card)
{
	struct mt8168_onyx_priv *card_data =
		snd_soc_card_get_drvdata(card);
...
/* HP Spk Amp */
static int mt8168_onyx_hp_spk_amp_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		mt8168_onyx_ext_hp_amp_spk_turn_on(card);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt8168_onyx_ext_hp_amp_spk_turn_off(card);
		break;
	default:
		break;
	}
...
static void mt8168_onyx_ext_hp_amp_turn_on(struct snd_soc_card *card)
{
	struct mt8168_onyx_priv *card_data =
		snd_soc_card_get_drvdata(card);
...
static void mt8168_onyx_ext_hp_amp_turn_off(struct snd_soc_card *card)
{
	struct mt8168_onyx_priv *card_data =
		snd_soc_card_get_drvdata(card);
...
/* HP Amp */
static int mt8168_onyx_hp_amp_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_dapm_context *dapm = w->dapm;
	struct snd_soc_card *card = dapm->card;
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		mt8168_onyx_ext_hp_amp_turn_on(card);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt8168_onyx_ext_hp_amp_turn_off(card);
		break;
	default:
		break;
	}
...
static const struct snd_kcontrol_new mt8168_onyx_hpl_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_hpr_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_lol_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_hpl_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_hpr_output_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_hpl_spk_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);
static const struct snd_kcontrol_new mt8168_onyx_hpr_spk_switch_ctrl =
	SOC_DAPM_SINGLE_VIRT("Switch", 1);

static const struct snd_soc_dapm_widget mt8168_onyx_widgets[] = {
	SND_SOC_DAPM_SWITCH("AU_HPL",
		SND_SOC_NOPM, 0, 0, &mt8168_onyx_hpl_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_HPR",
		SND_SOC_NOPM, 0, 0, &mt8168_onyx_hpr_switch_ctrl),
	SND_SOC_DAPM_SWITCH("AU_LOL",
		SND_SOC_NOPM, 0, 0, &mt8168_onyx_lol_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Output",
		SND_SOC_NOPM, 0, 0, &mt8168_onyx_hpl_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Output",
		SND_SOC_NOPM, 0, 0, &mt8168_onyx_hpr_output_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_L Spk Output",
		SND_SOC_NOPM, 0, 0,
		&mt8168_onyx_hpl_spk_switch_ctrl),
	SND_SOC_DAPM_SWITCH("HP_R Spk Output",
		SND_SOC_NOPM, 0, 0,
		&mt8168_onyx_hpr_spk_switch_ctrl),

	SND_SOC_DAPM_SPK("HP Spk Amp", mt8168_onyx_hp_spk_amp_event),
	SND_SOC_DAPM_SPK("LOL Spk Amp", NULL),
	SND_SOC_DAPM_MIC("PMIC MIC", NULL),
	SND_SOC_DAPM_HP("Headphone", mt8168_onyx_hp_amp_event),
#ifdef TEST_BACKEND_WITH_ENDPOINT
	SND_SOC_DAPM_OUTPUT("TDM_OUT Out"),
#endif
};

static const struct snd_soc_dapm_route mt8168_onyx_routes[] = {
...
static int mt8168_be_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
	struct snd_pcm_hw_params *params)
{
	struct mt8168_onyx_priv *priv =
		snd_soc_card_get_drvdata(rtd->card);
	int id = rtd->dai_link->id;
	struct mt8168_onyx_tdm_ctrl_data *tdm;
	struct mt8168_onyx_dmic_ctrl_data *dmic;
...
static int mt8168_onyx_int_adda_startup(
	struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8168_onyx_priv *priv =
...
static void mt8168_onyx_int_adda_shutdown(
	struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8168_onyx_priv *priv =
...
static struct snd_soc_ops mt8168_onyx_int_adda_ops = {
	.startup = mt8168_onyx_int_adda_startup,
	.shutdown = mt8168_onyx_int_adda_shutdown,
};

/* Digital audio interface glue - connects codec <---> CPU */
static struct snd_soc_dai_link mt8168_onyx_dais[] = {
	/* Front End DAI links */
...
static int mt8168_onyx_gpio_probe(struct snd_soc_card *card)
{
	struct mt8168_onyx_priv *priv = snd_soc_card_get_drvdata(card);
	...
	for (i = 0 ; i < PIN_STATE_MAX ; i++) {
		priv->pin_states[i] = pinctrl_lookup_state(priv->pinctrl,
			mt8168_onyx_pin_str[i]);
		if (IS_ERR(priv->pin_states[i])) {
			ret = PTR_ERR(priv->pin_states[i]);
			dev_info(card->dev, "%s Can't find pin state %s %d\n",
				 __func__, mt8168_onyx_pin_str[i], ret);
		}
	}
	...
	for (i = PIN_STATE_DEFAULT ; i < PIN_STATE_MAX ; i++) {
		if (IS_ERR(priv->pin_states[i])) {
			dev_info(card->dev, "%s check pin %s state err\n",
				 __func__, mt8168_onyx_pin_str[i]);
			continue;
		}
...
static void mt8168_onyx_parse_of(struct snd_soc_card *card,
...
static struct snd_soc_card mt8168_onyx_card = {
	.name = "mt-snd-card",
	.owner = THIS_MODULE,
	.dai_link = mt8168_onyx_dais,
	.num_links = ARRAY_SIZE(mt8168_onyx_dais),
	.controls = mt8168_onyx_controls,
	.num_controls = ARRAY_SIZE(mt8168_onyx_controls),
	.dapm_widgets = mt8168_onyx_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8168_onyx_widgets),
	.dapm_routes = mt8168_onyx_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8168_onyx_routes),
};

static int mt8168_onyx_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt8168_onyx_card;
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (is_adsp_dai_link(i))
			continue;
#endif
		if (mt8168_onyx_dais[i].platform_name)
			continue;
		mt8168_onyx_dais[i].platform_of_node = platform_node;
	}
#ifdef CONFIG_SND_SOC_RT551X
	if (platform_node)
	...
	for (i = 0; i < ARRAY_SIZE(adsp_dai_links); i++) {
		...
		if (mt8168_onyx_dais[dai].platform_name)
			continue;
		mt8168_onyx_dais[dai].platform_of_node = platform_node;
	}
#endif

	card->dev = dev;

	priv = devm_kzalloc(dev, sizeof(struct mt8168_onyx_priv),
			    GFP_KERNEL);
	if (!priv) {
		ret = -ENOMEM;
	...
	mt8168_onyx_gpio_probe(card);

	mt8168_onyx_parse_of(card, np);
...
static const struct of_device_id mt8168_onyx_dt_match[] = {
	{ .compatible = "mediatek,mt8168-onyx", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8168_onyx_dt_match);

static struct platform_driver mt8168_onyx_driver = {
	.driver = {
		   .name = "mt8168-onyx",
		   .of_match_table = mt8168_onyx_dt_match,
#ifdef CONFIG_PM
		   .pm = &snd_soc_pm_ops,
#endif
	},
	.probe = mt8168_onyx_dev_probe,
};

module_platform_driver(mt8168_onyx_driver);

/* Module information */
MODULE_DESCRIPTION("MT8168 Onyx SoC machine driver");
MODULE_AUTHOR("Jia Zeng <jia.zeng@mediatek.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:mt8168-onyx");
