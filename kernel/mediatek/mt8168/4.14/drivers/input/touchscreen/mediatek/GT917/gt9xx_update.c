...
static u8 gup_enter_update_judge(struct st_fw_head *fw_head)
{
	...
	/* 3. the firmware version in update file is greater */
	/* than the firmware */
	/* version in ic */
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	/* GTP_DEBUG("[burn_fw_ss51]step2:load ss51 */
	/* firmware section 1 file data"); */
...
#if 0
		GTP_ERROR(
			"Inconsistent firmware size. File size: %d, default fw size: %l",
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_touch_fw[FW_HEAD_LENGTH],
			      FW_DOWNLOAD_LENGTH);
...
s32 gup_load_authorization_fw(void)
{
	...
	if (section == NULL) {
		section = gtp_hotknot_authorization_fw;
		GTP_INFO(
			"[gup load_authorization_fw] Use default calibration2.");
	} else {
		is_load_from_file = 1;
	}
#endif
...
s32 gup_recovery_touch(void)
{
	...
	ret = gup_load_calibration(&section[CALIBRATION_HEADER_LEN],
