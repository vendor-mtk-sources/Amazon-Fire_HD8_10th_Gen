...
static void hall_work_func(struct work_struct *work)
{
	...
	unsigned int gpio_pin = priv->sensor_data[0].gpio_pin;
	...
	priv->cover =
		priv->sensor_data[0].gpio_state ? COVER_OPENED : COVER_CLOSED;
...
static int configure_irqs(struct hall_priv *priv)
{
	...
	for (i = 0; i < priv->hall_sensor_num; i++) {
		...
		irq_set_irq_type(sensor_data[i].irq,
				priv->sensor_data[i].gpio_state ?
				IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING);
	}
...
static int hall_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < hall_sensors->hall_sensor_num; i++) {
		...
		hall_sensors->hall_sensor_data[i].irq =
					irq_of_parse_and_map(of_node, i);
