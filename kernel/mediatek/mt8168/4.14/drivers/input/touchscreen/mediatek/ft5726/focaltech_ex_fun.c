...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	if (i_ret  < 0)
		pr_debug("[focal] failed to write FACTORYMODE_VALUE\n");
	...
	if (i_ret  < 0)
		pr_debug("[focal] failed to write 0x4\n");
	...
	for (i = 0; i < 100; i++) {
		...
		if (i_ret  < 0)
			pr_debug("[focal] failed to read mode info\n");
	...
	if (i_ret  < 0)
		pr_debug("[focal] failed to goto factory mode for store\n");
	...
	if (i_ret  < 0)
		pr_debug("[focal] failed to store CLB result\n");

	msleep(300);
	/*return to normal mode */
	i_ret = fts_write_reg(client, 0, FTS_WORKMODE_VALUE);
	if (i_ret  < 0)
		pr_debug("[focal] failed to return to normal mode\n");
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if (ft_vendor_id == FT_LENS_ID || ft_vendor_id == FT_LENS_ID_PROTO) {
		fw_len = ARRAY_SIZE(CTPM_FW_LENS);
		pbt_buf = CTPM_FW_LENS;
	} else if (ft_vendor_id == FT_TopGroup_ID) {
		fw_len = ARRAY_SIZE(CTPM_FW_TopGroup);
		pbt_buf = CTPM_FW_TopGroup;
	} else {
		pr_debug("[focal] %s unknown ft_vendor_id:0x%x\n",
		__func__, ft_vendor_id);
		goto err;
	}

	if (fw_len < 8 || fw_len > 54 * 1024) {
		dev_dbg(&client->dev, "[focal] %s:FW length error\n", __func__);
		return -EIO;
	}
...
u8 fts_ctpm_get_i_file_ver(void)
{
	...
	if (ft_vendor_id == FT_LENS_ID || ft_vendor_id == FT_LENS_ID_PROTO) {
		ui_sz = ARRAY_SIZE(CTPM_FW_LENS);
		pbt_buf = CTPM_FW_LENS;
	} else if (ft_vendor_id == FT_TopGroup_ID) {
		ui_sz = ARRAY_SIZE(CTPM_FW_TopGroup);
		pbt_buf = CTPM_FW_TopGroup;
	} else {
		pr_debug("[focal] %s unknown ft_vendor_id:0x%x\n",
			__func__, ft_vendor_id);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if ((uc_tp_fm_ver < uc_host_fm_ver) || (uc_tp_fm_ver >= 0xe0) ||
		...
		if (i_ret == 0) {
			...
			pr_debug("[focal] Auto update failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
	return 0;
...
int hid_to_i2c(struct i2c_client *client)
{
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "[focal] %s: write iic error\n",
			__func__);
		return ret;
	}
	msleep(20);
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "[focal] %s:  read iic error\n",
			__func__);
		return ret;
	}

	if ((auc_i2c_write_buf[0] == 0xeb) &&
	    (auc_i2c_write_buf[1] == 0xaa) && (auc_i2c_write_buf[2] == 0x08)) {
		pr_debug("[focal] HidI2c_To_StdI2c successful\n");
		bRet = 1;
	} else {
		pr_debug("[focal] HidI2c_To_StdI2c error.\n");
		bRet = 0;
	}
			     ...
			     u8 *pbt_buf, u32 dw_length)
{
	...
	pr_info("[focal] Entering fts 5x22_ctpm_fw_upgrade ....\n");

	i_ret = hid_to_i2c(client);
	if (i_ret == 0)
		pr_debug("[focal] hid change to i2c fail\n");
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (i_ret  < 0)
			pr_debug("[focal] failed to write 0xaa to register 0xfc\n");

		msleep(IC_INFO_DELAY_AA);

		i_ret = fts_write_reg(client, 0xfc, FTS_UPGRADE_55);
		if (i_ret < 0)
			pr_debug("[focal] failed to write 0x55 to register 0xfc\n");
		...
		if (i_ret < 0) {
			pr_debug("[focal] failed writing 0x55 and 0xaa\n");
			continue;
		}
		...
		i_ret =
			ftxxxx_i2c_Read(client, auc_i2c_write_buf,
			4, reg_val, 2);
		if (i_ret < 0)
			pr_debug("[focal] fail on checking read-ID\n");
	...
	for (i = 0; i < 15; i++) {
		...
		if (i_ret < 0)
			pr_debug("[focal] failed to read iic\n");
	...
	if (i_ret < 0)
		pr_debug("[focal] failed to write bin file length\n");
	...
	for (j = 0; j < packet_number; j++) {
		...
		if (i_ret < 0)
			pr_debug("[focal] failed to write package\n");
		...
		for (i = 0; i < 30; i++) {
			...
			i_ret = ftxxxx_i2c_Read(client, auc_i2c_write_buf, 1,
						reg_val, 2);
			if (i_ret < 0)
				pr_debug("[focal] failed to read from 0x6a\n");
	...
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
		...
		if (i_ret < 0)
			pr_debug("[focal] failed to write i2c\n");
	...
	i_ret = ftxxxx_i2c_Write(client, auc_i2c_write_buf, 1);
	if (i_ret < 0)
		pr_debug("[focal] failed to read from 0x64\n");
	...
	if (i_ret < 0)
		pr_debug("[focal] failed to read from 0x65\n");
	...
	if (i_ret < 0)
		pr_debug("[focal] failed to read from 0x66\n");
	...
	i_ret = ftxxxx_i2c_Write(client, auc_i2c_write_buf, 1);
	if (i_ret < 0)
		pr_debug("[focal] failed to reset the new FW\n");

	msleep(200);		/* make sure CTP startup normally */

	i_ret = hid_to_i2c(client);
	if (i_ret == 0)
		pr_debug("[focal] HidI2c change to StdI2c fail !\n");
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	if (IS_ERR(pfile)) {
		pr_debug("[focal] error occurred while opening file %s.\n",
		       filepath);
		return -EIO;
	}
...
static int ftxxxx_get_testparam_from_ini(char *config_name)
{
	...
	if (inisize <= 4096) {
		pr_debug("%s ERROR:Get firmware size failed\n", __func__);
		return -EIO;
	}
	...
	if (ftxxxx_ReadInIData(config_name, filedata)) {
		pr_debug("[focal] %s() - ERROR: request_firmware failed\n",
			__func__);
		kfree(filedata);
		return -EIO;
	}
						...
						const char *buf, size_t count)
{
	...
	if (ftxxxx_get_testparam_from_ini(cfgname) < 0)
		pr_debug("[focal] get testparam from ini failure %s\n",
	cfgname);
	else {
		pr_debug("[focal] TP test Start...\n");
		if (true == start_test_tp()) {
			g_test = 1;
			pr_debug("[focal] TP test pass\n");
		} else {
			g_test = 0;
			pr_debug("[focal] TP test failure\n");
		}

		iTestDataLen = get_test_data(testdata);
		pr_debug("[focal] test data = %s\n", testdata);
...
static DEVICE_ATTR(ftstpfwver, 0444, fts_tpfwver_show, NULL);


static DEVICE_ATTR(ftsfwupdate, 0220, NULL, fts_fwupdate_store);


static DEVICE_ATTR(ftstprwreg, 0220, NULL, fts_tprwreg_store);

static DEVICE_ATTR(ftsfwupgradeapp, 0220, NULL,
		   fts_fwupgradeapp_store);


static DEVICE_ATTR(ftsscaptest, 0220, NULL,
...
int fts_create_sysfs(struct i2c_client *client)
{
	...
	if (err != 0) {
		pr_debug("[focal] %s - ERROR: sysfs_create_group() failed.\n",
			__func__);
	...
	if (err == 0) {
		pr_debug("[focal] %s - ERROR: hid_to_i2c failed.\n", __func__);
		return -EIO;
	}
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(&writebuf, buf, buflen)) {
		dev_dbg(&client->dev, "[focal] %s:copy from user error\n",
			__func__);
		return -EFAULT;
	}
...
int fts_create_apk_debug_channel(struct i2c_client *client)
{
	...
	if (fts_proc_entry == NULL) {
		dev_dbg(&client->dev, "[focal] Couldn't create proc entry!\n");
		return -ENOMEM;
	}
	dev_dbg(&client->dev, "[focal] Create proc entry success!\n");
