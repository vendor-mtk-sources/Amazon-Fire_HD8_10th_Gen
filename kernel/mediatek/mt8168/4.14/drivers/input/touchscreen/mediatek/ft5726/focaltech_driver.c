	...
	if (retry == IICReadWriteRetryTime) {
		...
		if (IICErrorCountor >= 10) {
			pr_info
			    ("[focal] %s: i2c read/write error over 10 times\n",
			     __func__);
			pr_info("[focal] %s: excute reset IC process\n",
			       __func__);
			return ret;
		}
		return ret;
...
static void tpd_power_down(void)
{
	...
	msleep(20);
		...
		if (ret != 0)
			pr_info("[focal]Failed to disable VIO: %d\n", ret);
#endif

	ret = regulator_disable(tpd->reg);
	if (ret != 0)
		pr_info("[focal]Failed to disable VTP: %d\n", ret);
...
static void tpd_power_on(void)
{
	...
	msleep(20);
		...
		if (ret != 0)
			pr_info("[focal] Failed to enable VIO: %d\n", ret);
#endif

	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		pr_info("[focal] Failed to enable vtp: %d\n", ret);
...
static int of_get_focal_platform_data(struct device *dev)
{
	if (dev->of_node) {
		...
		if (!match) {
			pr_info("[focal] %s: No device match found\n",
			__func__);
			return -ENODEV;
		}
	}
...
static char get_hw_rev(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		match = of_match_device(of_match_ptr(focal_dt_match), dev);
		if (!match) {
			pr_info("[focal] No of-device match found\n");
			return 0;
		}
	}
	...
	if (IS_ERR(hwid_gpio_pinctrl)) {
		pr_info("[focal] get pinctrl failed, return hw_rev 0\n");
		return 0;
	}
	...
	if (retval < 0) {
		pr_info("[focal] set pinctrl state failed, return hw_rev 0\n");
		return 0;
	}
	...
	if (hwid_gpio[0] < 0) {
		pr_info("[focal] Can't find hwid0-gpio\n");
		return 0;
	}
	...
	if (hwid_gpio[1] < 0) {
		pr_info("[focal] Can't find hwid1-gpio\n");
		return 0;
	}
	...
	if (hwid_gpio[2] < 0) {
		pr_info("[focal] Can't find hwid2-gpio\n");
		return 0;
	}
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (IS_ERR(tpd->reg)) {
		pr_info("[focal] Can't request VTP power supply: %ld\n",
			   PTR_ERR(tpd->reg));
	...
	if (IS_ERR(tpd->io_reg))
		pr_info("[focal] Can't request VIO power supply\n");

	retval = regulator_set_voltage(tpd->io_reg, 1800000, 1800000);
	if (retval != 0) {
		pr_info("[focal] Failed to set VIO voltage: %d\n",
			retval);
		goto err_free_vio;
	}
#endif
	...
	/* pr_info("[focal] HW rev:%d\n", hw_rev); */

	retval = of_get_focal_platform_data(&i2c_client->dev);
	if (retval) {
		pr_info("[focal] GPIO configuration failed\n");
#ifdef CONFIG_TOUCHSCREEN_DUAL_POWER
		goto err_free_vio;
#else
		goto err_free_vtp;
	...
	if (retval) {
		pr_info("[focal] Init_gpio_hw failed\n");
#ifdef CONFIG_TOUCHSCREEN_DUAL_POWER
		goto err_free_vio;
#else
		goto err_free_vtp;
	...
	if (retval < 0 || reg_value[0] != 0) {
		...
		pr_info("[focal] Chip does not exits. I2C transfer error:%d, Reg0:%d\n",
			retval, reg_value[0]);
		goto err_free_hw;
	}
	...
	if (retval < 0)
		pr_info("[focal] Read FW ver error!\n");
	else
	...
	retval = ftxxxx_i2c_Read(i2c_client, &reg_addr, 1, reg_value, 1);
	if (retval < 0)
		pr_info("[focal] Read Vendor ID error!\n");
	else {
		ft_vendor_id = reg_value[0];
		pr_info("[focal] Vendor ID:0x%x\n", reg_value[0]);
	}

	reg_addr = FTS_REG_ROUTING_TYPE;
	retval = ftxxxx_i2c_Read(i2c_client, &reg_addr, 1, reg_value, 1);
	if (retval < 0)
		pr_info("[focal]Read routing type error!\n");
	else {
	...
	if (retval < 0)
		pr_info("[focal] Read Chip ID error!\n");
	else
	...
	if (fs != 0)
		pr_info("[focal] %s - ERROR: create sysfs failed.\n", __func__);
#endif

#ifdef FTS_CTL_IIC
	chdev = fts_rw_iic_drv_init(i2c_client);
	if (chdev < 0)
		pr_info("[focal] %s: create fts control iic driver failed\n",
		       __func__);
#endif
	...
	if (proc != 0)
		pr_info("[focal] Couldn't create apk debug channel!\n");
#endif
	...
	if (retval != 0)
		pr_info("[focal] Auto upgrade failed\n");
	...
	if (gtp_esd_check_workqueue == NULL) {
		pr_info("[focal] %s: couldn't create ESD workqueue\n",
		__func__);
		retval = -ENOMEM;
		goto err_free;
	}
...
static void force_reset_guitar(void)
{
	...
	pr_info("[focal] force reset_guitar\n");
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	pr_info("[focal] 0x8F:%d, count_irq is %d\n", data, count_irq);
	flag_error = 0;
	...
	if (flag_error == 1) {
		pr_info("[focal] tpd reset. data=%d, count_irq=%d\n",
		       data, count_irq);
		force_reset_guitar();
		reset_flag = 1;
...
FOCAL_RESET_A3_REGISTER:
	...
	if (ret < 0)
		pr_info("[focal] write value fail");
