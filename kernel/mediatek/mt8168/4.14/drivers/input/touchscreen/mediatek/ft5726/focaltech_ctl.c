...
static int fts_rw_iic_drv_myread(struct i2c_client *client, u8 *buf,
				 int length)
{
	int ret = 0;
	...
	if (ret < 0)
		dev_dbg(&client->dev, "[focal] %s:IIC Read failed\n", __func__);
				  ...
				  int length)
{
	...
	if (ret < 0)
		dev_dbg(&client->dev, "[focal] %s:IIC Write failed\n",
			__func__);
...
static int fts_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	i2c_rw_msg =
	    kmalloc_array(i2c_rw_queue.queuenum, sizeof(struct fts_rw_i2c),
		    GFP_KERNEL);
	if (!i2c_rw_msg)
		return -ENOMEM;
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (!fts_rw_i2c_dev_tt) {
		...
		dev_dbg(&client->dev, "[focal] %s: failed\n", __func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) {
		dev_dbg(&client->dev, "[focal] %s:failed in creating class.\n",
			__func__);
		return -1;
	}
	/*create device node */
...
{
	dev_dbg(&client->dev, "[focal] ft_rw_iic_drv init\n");
