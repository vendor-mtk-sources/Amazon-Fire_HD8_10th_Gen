		...
		const enum mtk_ddp_comp_id *path, unsigned int path_len)
{
	...
	if (priv->num_pipes == 0U) {
		...
		mtk_crtc->cmdq_event = cmdq_dev_get_event(
					priv->mutex_dev, "stream_done_0");
	} else if (priv->num_pipes == 1U) {
		mtk_crtc->cmdq_client = cmdq_mbox_create(dev, 1);
		mtk_crtc->cmdq_event = cmdq_dev_get_event(
					priv->mutex_dev, "stream_done_1");
	} else {
		mtk_crtc->cmdq_client = cmdq_mbox_create(dev, 2);
		mtk_crtc->cmdq_event = cmdq_dev_get_event(
					priv->mutex_dev, "stream_done_2");
	}
...
#ifdef CONFIG_MTK_DISPLAY_CMDQ
	if (priv->mutex_dev) {
		mtk_crtc->mutex_cmdq_base = cmdq_register_device(
						priv->mutex_dev);
		MTK_DRM_DEBUG_DRIVER("mutex_cmdq_base 0x%p\n",
		mtk_crtc->mutex_cmdq_base);
	} else {
		mtk_crtc->mutex_cmdq_base = 0x0;
		dev_err(dev, "mutex_dev is invalid!\n");
	}
#endif
