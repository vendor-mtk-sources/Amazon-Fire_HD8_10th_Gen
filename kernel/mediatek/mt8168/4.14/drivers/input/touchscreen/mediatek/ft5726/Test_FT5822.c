...
boolean FT5822_StartTest(void)
{
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++) {
		...
		if (g_stTestItem[ucDevice][iItemCount].ItemCode ==
	...
	for (iItemCount = 0; iItemCount <= 3; iItemCount++) {
		...
		SysDelay(50);
		pr_debug("[focal] Enter_WorkMode FAIL! (%d)\n",
			       iItemCount);
	}
...
unsigned char FT5822_TestItem_EnterFactoryMode(void)
{
	...
	for (i = 1; i <= iRedo; i++) {
		ReCode = EnterFactory();
		if (ReCode != ERROR_CODE_OK) {
			pr_debug("Failed to Enter factory mode...\n");
			if (i < iRedo) {
				SysDelay(50);
				continue;
			}
		...
		/*throw away three frames after reg value changed */
			...
			for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
				for (iCol = 0; iCol < g_ScreenSetParam.iRxNum;
				     iCol++) {
						/*Invalid Node */
					if (g_MCap.InvalidNode[iRow][iCol] == 0)
						continue;
					RawDataMin =
					    g_MCap.RawDataTest_High_Min
					    [iRow][iCol];
...
unsigned char FT5822_TestItem_SCapRawDataTest(bool *bTestResult)
{
	...
	ReCode = StartScan();
	if (ReCode != ERROR_CODE_OK) {
		pr_debug("Failed to Scan SCap RawData!\n");
		goto TEST_ERR;
	}
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapRawDataTest_SetWaterproof_OFF
		...
		if (bFlag)
			pr_debug("Judge Rx in Waterproof-OFF:\n");
	...
	if (m_bV3TP) {
		...
		if (ucValue != 0) {
			...
			SysDelay(10);
			if (ReCode != ERROR_CODE_OK) {
				pr_debug("Failed to switch mapping type!\n ");
				btmpresult = false;
			}
		}
...
unsigned char FT5822_TestItem_SCapCbTest(bool *bTestResult)
{
	...
	if (bFlag) {
		pr_debug("Failed to SwitchToNoMapping!\n");
		goto TEST_ERR;
	}
	...
	for (i = 0; i < 3; i++) {
		...
		if (ReCode != ERROR_CODE_OK)
			pr_debug("Failed to Get SCap CB!\n");
	}
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_ON
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++) {
			...
			CBMin =
			    g_MCap.SCapCbTest_ON_Min[0]
			    [i];
			CBMax =
			    g_MCap.SCapCbTest_ON_Max[0]
			    [i];
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iTxNum; i++) {
			...
			CBMin =
			    g_MCap.SCapCbTest_ON_Min[1]
			    [i];
			CBMax =
			    g_MCap.SCapCbTest_ON_Max[1]
			    [i];
	...
	if (g_stCfg_FT5822_BasicThreshold.SCapCbTest_SetWaterproof_OFF
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++) {
			...
			CBMin =
			    g_MCap.SCapCbTest_OFF_Min[0]
			    [i];
			CBMax =
			    g_MCap.SCapCbTest_OFF_Max[0]
			    [i];
			Value = m_RawData[2 + g_ScreenSetParam.iTxNum][i];
			iAvg += Value;
		...
		for (i = 0; bFlag && i < g_ScreenSetParam.iTxNum; i++) {
			...
			CBMin =
			    g_MCap.SCapCbTest_OFF_Min[1]
			    [i];
			CBMax =
			    g_MCap.SCapCbTest_OFF_Max[1]
			    [i];
...
unsigned char FT5822_TestItem_PanelDifferTest(bool *bTestResult)
{
	...
	if (ReCode != ERROR_CODE_OK) {
		pr_debug("Read REG_NORMALIZE_TYPE error.Error Code: %d\n",
		       ReCode);
		btmpresult = false;
		goto TEST_ERR;
	}
	...
	if (OriginRawDataType != 0) {
		...
		if (ReCode != ERROR_CODE_OK) {
			btmpresult = false;
			pr_debug("Write reg REG_NORMALIZE_TYPE Failed.\n");
			goto TEST_ERR;
		}
	}
	...
	if (ReCode != ERROR_CODE_OK) {
		pr_debug("Read reg 0x0A error. Error Code: %d\n", ReCode);
		btmpresult = false;
		goto TEST_ERR;
	}
	...
	if (ReCode != ERROR_CODE_OK) {
		btmpresult = false;
		pr_debug("Write reg 0x0A Failed.\n");
		goto TEST_ERR;
	}
	...
	if (ReCode != ERROR_CODE_OK) {
		pr_debug("Read reg 0xFB error. Error Code: %d", ReCode);
		btmpresult = false;
		goto TEST_ERR;
	}
	...
	SysDelay(50);
	if (ReCode != ERROR_CODE_OK) {
		pr_debug("Write reg 0xFB Failed.");
		btmpresult = false;
		goto TEST_ERR;
	}
	...
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
			...
			iMin =
			g_MCap.PanelDifferTest_Min[iRow][iCol];
			iMax =
			g_MCap.PanelDifferTest_Max[iRow][iCol];
	...
	for (i = 0; i < g_ScreenSetParam.iTxNum; i++) {
		for (j = 0; j < g_ScreenSetParam.iRxNum; j++) {
			pr_debug("%d,", abs(m_DifferData[i][j]));
...
static unsigned char GetChannelNum(void)
{
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		pr_debug("Failed to get Tx number\n");
	}
	...
	if (ReCode == ERROR_CODE_OK) {
		...
		pr_debug("Failed to get Rx number\n");
	}
...
static unsigned char SwitchToNoMapping(void)
{
	...
	if (ReCode != ERROR_CODE_OK)
		pr_debug("Switch To NoMapping Failed!\n");
		...
		for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; ++iRow) {
			for (iCol = 1; iCol < g_ScreenSetParam.iRxNum; ++iCol) {
				...
				if (TxLinearity[iRow][iCol] < MIN_HOLE_LEVEL
				    || TxLinearity[iRow][iCol] >
		g_stCfg_FT5822_BasicThreshold.Uniformity_Tx_Hole) {
					pr_debug
		...
		for (iRow = 1; iRow < g_ScreenSetParam.iTxNum; ++iRow) {
			for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; ++iCol) {
				...
				if (RxLinearity[iRow][iCol] < MIN_HOLE_LEVEL
				    || RxLinearity[iRow][iCol] >
		g_stCfg_FT5822_BasicThreshold.Uniformity_Rx_Hole) {
					pr_debug
