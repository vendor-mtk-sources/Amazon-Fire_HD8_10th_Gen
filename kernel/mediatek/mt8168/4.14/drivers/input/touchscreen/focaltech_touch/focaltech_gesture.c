    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = scnprintf(buf, PAGE_SIZE, "Gesture Set Mode:%s, Gesture Now Mode:%s\n",
			fts_gesture_data.ex_mode ? "On" : "Off",
			fts_gesture_data.mode ? "On" : "Off");
    count += scnprintf(buf + count, PAGE_SIZE, "Reg(0xD0)=%d\n", val);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fts_gesture_data.ex_mode = ENABLE;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
	FTS_DEBUG("disable gesture");
	fts_gesture_data.ex_mode = DISABLE;
    }
    mutex_unlock(&input_dev->mutex);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&input_dev->mutex);
    count = scnprintf(buf, PAGE_SIZE, "Gesture ID:%d\n", gesture->gesture_id);
    count += scnprintf(buf + count, PAGE_SIZE, "Gesture PointNum:%d\n",
			gesture->point_num);
    count += scnprintf(buf + count, PAGE_SIZE, "Gesture Points Buffer:\n");

    /* save point data,max:6 */
    for (i = 0; i < FTS_GESTURE_POINTS_MAX; i++) {
	count += scnprintf(buf + count, PAGE_SIZE, "%3d(%4d,%4d) ", i,
...
/* sysfs gesture node
 *   read example: cat  gesture_wakeup      ---read gesture mode
 *   write example:echo 1 > gesture_wakeup   --- write gesture mode to 1
 *
 */
static DEVICE_ATTR(gesture_wakeup, S_IRUGO | S_IWUSR, fts_gesture_show,
			fts_gesture_store);
/*
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
	...
	gesture->coordinate_y[i] = (u16)(((buf[2 + index] & 0x0F) << 8)
					+ buf[3 + index]);
	if (1 == ts_data->pdata->swap_x2y) {
		swap_coordinate = gesture->coordinate_x[i];
		gesture->coordinate_x[i] = ts_data->pdata->x_max - gesture->coordinate_y[i];
		gesture->coordinate_y[i] = swap_coordinate;
	}
    }
...
int fts_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    fts_gesture_data.mode = fts_gesture_data.ex_mode;
