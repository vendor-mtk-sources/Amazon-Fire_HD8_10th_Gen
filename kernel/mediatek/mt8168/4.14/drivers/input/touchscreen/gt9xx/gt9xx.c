				    ...
				    size_t size, loff_t *ppos)
{
	...
	data_len += snprintf(ptr + data_len, PROP_NAME_SIZE - data_len,
			"goodix sid:0x%02X fw_ver:0x%02X cfg_ver:0x%02X\n",
			fw_info->sensor_id, fw_info->version, config_version);
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	gtp_firmware_proc = proc_create(GT91XX_FIRMWARE_PROC_FILE, 0664,
