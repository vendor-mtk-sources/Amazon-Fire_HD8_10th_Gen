...
static int accelgyro_probe(void)
{
	...
	if (!acc_context_obj) {
		...
		goto exit_acc_alloc_data_failed;
	}
	/*gyro initialize */
	...
	if (!gyro_context_obj) {
		err = -ENOMEM;
		pr_err("unable to allocate devobj!\n");
		goto exit_gyro_alloc_data_failed;
	}
	...
	if (!timer_obj) {
		...
		goto accelgyro_timer_alloc_failed;
	}
	...
	if (!timer_obj->accelgyro_workqueue) {
		err = -EFAULT;
		pr_err("create accelgyro workqueue error!\n");
		goto accelgyro_create_workqueue_failed;
	}
...
real_driver_init_fail:
	destroy_workqueue(timer_obj->accelgyro_workqueue);
accelgyro_create_workqueue_failed:
	kfree(timer_obj);
accelgyro_timer_alloc_failed:
	kfree(gyro_context_obj);
exit_gyro_alloc_data_failed:
	kfree(acc_context_obj);
exit_acc_alloc_data_failed:
