...
#define PROC_FW_NAME                            "tp_fw_version"
...
static ssize_t ft_fw_read_proc(struct file *file, char __user *page,
				    size_t size, loff_t *ppos)
{
    ...
    ptr = kzalloc(PROC_BUF_SIZE, GFP_KERNEL);
    if (!ptr) {
	FTS_ERROR("Failed alloc memory for config\n");
	return -ENOMEM;
    }
    ...
    fts_esdcheck_proc_busy(1);
#endif
    ret = fts_read_reg(FTS_REG_FW_VER, &val);
	if (ret < 0)
		FTS_ERROR("read fw version\n");
    FTS_DEBUG("fts_read_reg = %x", val);

    ret = fts_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
	if (ret < 0)
		FTS_ERROR("read vendor_id\n");
    ret = fts_read_reg(FTS_REG_PANEL_ID, &panel_id);
	if (ret < 0)
		FTS_ERROR("read panel_id\n");

#if FTS_ESDCHECK_EN
    fts_esdcheck_proc_busy(0);
#endif
    data_len += scnprintf(ptr + data_len, PROC_BUF_SIZE - data_len,
		"focal fw_ver:0x%02X ", val);
    data_len += scnprintf(ptr + data_len, PROC_BUF_SIZE - data_len,
		"pid:0x%02X ", panel_id);
    data_len += scnprintf(ptr + data_len, PROC_BUF_SIZE - data_len,
		"vid:0x%02X\n", vendor_id);
...
static const struct file_operations ft_fw_proc_fops = {
	.owner = THIS_MODULE,
	.read = ft_fw_read_proc,
};
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    ...
    struct ftxxxx_proc *proc = &ts_data->proc;
    struct ftxxxx_proc *proc_fw = &ts_data->proc_fw;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    ...
    proc_fw->proc_entry = proc_create(PROC_FW_NAME, 0664, NULL, &ft_fw_proc_fops);
    if (NULL == proc_fw->proc_entry) {
	FTS_ERROR("create proc fw entry fail");
	return -ENOMEM;
    }
#else
...
void fts_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
    ...
    if (proc_fw->proc_entry) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
	proc_remove(proc_fw->proc_entry);
#else
	remove_proc_entry(PROC_FW_NAME, NULL);
#endif
    }
}
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_read_reg(FTS_REG_PANEL_ID, &val);
    count += snprintf(buf + count, PAGE_SIZE, "Panel ID:0x%02x\n", val);
...
static DEVICE_ATTR(fw_version, S_IRUGO | S_IWUSR, fts_tpfwver_show, fts_tpfwver_store);
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
    ...
    /* Add sysfs Link*/
    ts_data->touchscreen_link = kobject_create_and_add("touchscreen", NULL);
    if (ts_data->touchscreen_link != NULL) {
	ret = sysfs_create_link(ts_data->touchscreen_link,
			&ts_data->dev->kobj,
			"link_input_dev");
	if (ret < 0)
		FTS_ERROR("Failed to create sysfs link for input_dev \n");
	}
    return ret;
}
