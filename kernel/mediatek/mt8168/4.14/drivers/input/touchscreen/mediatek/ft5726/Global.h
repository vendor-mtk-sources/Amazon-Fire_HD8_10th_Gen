...
struct StruScreenSeting {
	int iSelectedIC;/*Currently selected IC*/
	int iTxNum;
	int iRxNum;
	int isNormalize;
	int iUsedMaxTxNum;/*iTxNum<=iUsedMaxTxNum*/
	int iUsedMaxRxNum;/*RxNum<=iUsedMaxRxNum*/
...
struct stTestItem {
	 ...
	 * CfgItem, DataTestItem, GraphTestItem,
	 */
	unsigned char ItemType;
	unsigned char TestNum;/*serial number*/
	unsigned char TestResult;/*NG\PASS\TESTING*/
...
extern struct stCfg_MCap_DetailThreshold g_MCap;
extern struct stCfg_SCap_DetailThreshold g_SCap;
...
unsigned char Comm_Base_IIC_IO(unsigned char *pWriteBuffer,
	int  iBytesToWrite, unsigned char *pReadBuffer, int iBytesToRead);
