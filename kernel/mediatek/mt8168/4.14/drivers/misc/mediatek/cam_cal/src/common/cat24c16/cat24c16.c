...
static int iReadRegI2C(u8 *a_pSendData, u16 a_sizeSendData, u8 *a_pRecvData,
		u16 a_sizeRecvData, u16 i2cId)
{
	...
	g_pstI2Cclient->ext_flag =
		(g_pstI2Cclient->ext_flag) & (~I2C_DMA_FLAG);
#endif
	spin_unlock(&g_CAM_CALLock);
	i4RetValue = i2c_master_send(
		g_pstI2Cclient, a_pSendData, a_sizeSendData);
	if (i4RetValue != a_sizeSendData) {
		CAM_CALERR("I2C send failed!!, Addr = 0x%x\n", a_pSendData[0]);
		return -1;
	}
	i4RetValue = i2c_master_recv(
		g_pstI2Cclient, (char *)a_pRecvData, a_sizeRecvData);
...
static int iWriteReg(u16 a_u2Addr, u32 a_u4Data, u32 a_u4Bytes, u16 i2cId)
{
	...
	do {
		...
		if (i4RetValue != (a_u4Bytes + 2))
			CAM_CALERR(
			  " I2C send failed addr = 0x%x, data = 0x%x !!\n",
			  a_u2Addr, a_u4Data);
		else
			break;
		mdelay(5);
...
static bool selective_read_byte(u32 addr, u8 *data, u16 i2c_id)
{
	...
	if (iReadRegI2C(&offset, 1, (u8 *)data, 1,
		i2c_id + (page << 1)) < 0) {
		CAM_CALERR(
		"fail %s addr =0x%x data = 0x%x,page %d, offset 0x%x",
		 __func__, addr, *data, page, offset);
		return false;
	}
...
static int selective_read_region(u32 addr, u8 *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		...
		if (size_to_read > BUFF_SIZE) {
			...
			if (iReadRegI2C(&offset, 1, (u8 *)buff,
				BUFF_SIZE, i2c_id + (page << 1)) < 0)
				break;
			ret += BUFF_SIZE;
			buff += BUFF_SIZE;
			offset += BUFF_SIZE;
			...
			CAM_CALDB("offset =%x size %d\n", offset, size_to_read);
			if (iReadRegI2C(&offset, 1, (u8 *)buff,
				(u16) size_to_read, i2c_id + (page << 1)) < 0)
				break;
			ret += size_to_read;
			size_to_read = 0;
		}
	...
	CAM_CALDB("%s addr =%x size %d data read = %d\n",
...
static long cat24c16_Ioctl_Compat(
	struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	CAM_CALDB(
		"[CAMERA SENSOR] %s %p %p %x ioc size %d\n", __func__,
	...
	case COMPAT_CAM_CALIOC_G_READ: {
		...
		ret = filp->f_op->unlocked_ioctl(
			filp, CAM_CALIOC_G_READ, (unsigned long)data);
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		pBuff = kmalloc(
			sizeof(struct stCAM_CAL_INFO_STRUCT), GFP_KERNEL);
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param,
				...
				kfree(pBuff);
				CAM_CALDB(
				  "[S24CAM_CAL] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	CAM_CALDB(
	  " init Working buffer address 0x%p  command is 0x%x\n",
	...
	case CAM_CALIOC_G_READ:
		...
		/* CAM_CALDB(
		 *	"[CAM_CAL] Before read Working buffer address 0x%p\n",
		 *	pu1Params);
		 */

		i4RetValue = selective_read_region(
