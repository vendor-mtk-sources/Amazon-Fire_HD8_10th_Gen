...
static int rt9466_register_rt_regmap(struct rt9466_info *info)
{
	...
	if (!info->regmap_dev) {
		pr_debug("%s: register regmap dev fail\n", __func__);
		return -EIO;
	}
	...
	u8 data)
{
	...
	if (ret < 0)
		pr_debug("%s: I2CW[0x%02X] = 0x%02X fail\n",
			__func__, cmd, data);
	else
	...
	enum rt9466_adc_sel adc_sel, int *adc_val)
{
	...
	if (ret < 0) {
		pr_debug("%s: select ch to %d fail(%d)\n", __func__,
			adc_sel, ret);
		goto out;
	}
	...
	if (ret < 0) {
		pr_debug("%s: start con fail(%d), sel = %d\n",
	...
	if (i == max_wait_times) {
		pr_debug("%s: wait con fail(%d), sel = %d\n",
			__func__, ret, adc_sel);
		ret = -EINVAL;
		goto out_unlock_all;
	}
...
static int __rt9466_run_aicl(struct rt9466_info *info)
{
	...
	if (ret < 0) {
		pr_debug("%s: read mivr stat fail\n", __func__);
		goto out;
	}
...
static int rt9466_chg_mivr_irq_handler(struct rt9466_info *info)
{
	...
	ret = rt9466_i2c_test_bit(info, RT9466_REG_CHG_STATC,
		RT9466_SHIFT_CHG_MIVR, &mivr_act);
	if (ret < 0) {
		pr_debug("%s: read mivr stat failed\n", __func__);
		goto out;
	}
...
static int rt9466_irq_register(struct rt9466_info *info)
{
	...
	ret = devm_request_threaded_irq(info->dev, info->irq, NULL,
		...
		info);
	if (ret < 0) {
		pr_debug("%s: request thread irq fail\n", __func__);
		return ret;
	}
...
static bool rt9466_is_hw_exist(struct rt9466_info *info)
{
	...
	if (vendor_id != RT9466_VENDOR_ID) {
		pr_debug("%s: vendor id is incorrect (0x%02X)\n",
			__func__, vendor_id);
		return false;
	}
...
static int rt9466_enable_pump_express(struct rt9466_info *info, bool en)
{
	...
	if (ret < 0)
		pr_debug("%s: disable skip mode fail\n", __func__);
...
static int rt9466_parse_dt(struct rt9466_info *info, struct device *dev)
{
	...
	if (strcmp(desc->chg_dev_name, "secondary_chg") == 0) {
		...
		if (ret < 0) {
			pr_debug("%s: ceb gpio request fail\n",
				__func__);
			return ret;
		}
	}
	...
	if (of_property_read_u32(np, "regmap_represent_slave_addr",
		...
		pr_debug("%s: no regmap slave addr\n", __func__);

	if (of_property_read_string(np, "regmap_name",
		&(desc->regmap_name)) < 0)
		pr_debug("%s: no regmap name\n", __func__);
...
static int rt9466_enable_charging(struct charger_device *chg_dev, bool en)
{
	...
	if (strcmp(info->desc->chg_dev_name, "secondary_chg") == 0) {
		...
		if (ret < 0) {
			pr_debug("%s: set hz of sec chg fail\n",
				__func__);
			return ret;
		}
...
static int rt9466_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (ret < 0) {
		pr_debug("%s: set current limit fail\n", __func__);
		return ret;
	}
	...
	ret = rt9466_i2c_write_byte(info, RT9466_REG_CHG_HIDDEN_CTRL4,
		lg_slew_rate);
	if (ret < 0) {
		pr_debug("%s: set Low side mos Gate drive speed fail(%d)\n",
			__func__, ret);
		goto out;
	}
	...
	ret = rt9466_i2c_write_byte(info, RT9466_REG_CHG_HIDDEN_CTRL6,
		hidden_val);
	if (ret < 0)
		pr_debug("%s: workaroud fail(%d)\n", __func__, ret);

	/* Disable WDT */
	if (!en) {
		ret = rt9466_enable_wdt(info, false);
		if (ret < 0)
			pr_debug("%s: disable wdt fail\n", __func__);
	}
	goto out;
...
err_en_otg:
	...
	if (ret < 0)
		pr_debug("%s: recover Low side mos Gate drive speed fail(%d)\n",
			__func__, ret);
	ret = -EIO;
out:
	rt9466_enable_hidden_mode(info, false);
...
static int rt9466_enable_discharge(struct charger_device *chg_dev, bool en)
{
	...
	if (!en) {
		...
		if (i == check_dischg_max)
			pr_debug("%s: disable dischg fail(%d)\n",
				__func__, ret);
	}
...
static int rt9466_set_mivr(struct charger_device *chg_dev, u32 mivr)
{
	...
	if (ret < 0) {
		pr_debug("%s: get power path en fail\n", __func__);
		return ret;
	}
...
static int __rt9466_enable_auto_sensing(struct rt9466_info *info, bool en)
{
	...
	ret = rt9466_device_read(info->client, RT9466_REG_CHG_HIDDEN_CTRL15, 1,
		&auto_sense);
	if (ret < 0) {
		pr_debug("%s: read auto sense fail\n", __func__);
		goto out;
	}
...
static int rt9466_plug_out(struct charger_device *chg_dev)
{
	...
	if (strcmp(info->desc->chg_dev_name, "secondary_chg") == 0) {
		ret = rt9466_enable_hz(info, true);
		if (ret < 0)
			pr_debug("%s: en hz of sec chg fail\n",
				__func__);
	}
...
static int rt9466_dump_register(struct charger_device *chg_dev)
{
	...
	pr_info("%s: CHG_EN = %d, CHG_STATUS = %s, CHG_STAT = 0x%02X\n",
		__func__, chg_en, rt9466_chg_status_name[chg_status], chg_stat);

	pr_info("%s: CHG_CTRL1 = 0x%02X, CHG_CTRL2 = 0x%02X\n",
	...
	const struct i2c_device_id *dev_id)
{
	...
	if (ret < 0) {
		pr_debug("%s: reset chip fail\n", __func__);
		goto err_reset_chip;
	}

	ret = rt9466_init_setting(info);
	if (ret < 0) {
		pr_debug("%s: init setting fail\n", __func__);
	...
	if (ret < 0) {
		pr_debug("%s: irq register fail\n", __func__);
		goto err_irq_register;
	}

	ret = rt9466_irq_init(info);
	if (ret < 0) {
		pr_debug("%s: irq init fail\n", __func__);
		goto err_irq_init;
	}
...
static int __init rt9466_init(void)
{
	...
	if (ret < 0)
		pr_debug("%s: register i2c driver fail\n", __func__);
