...
static int life_cycle_reason_lookup(void)
{
	...
	life_cycle_reason_t cmdline_reason = LIFE_CYCLE_NOT_AVAILABLE;
	...
	if (!((p_dev_sol->sol_ops.read_boot_reason) &&
		...
		(p_dev_sol->sol_ops.read_from_cmdline))) {
	...
	p_dev_sol->sol_ops.read_from_cmdline(&cmdline_reason);
	...
	/* step 5, find cmdline mode reason if it's valid */
	if (cmdline_reason > 0) {
		index = search_lcr_table(lcr_data, table_num,
					 cmdline_reason);
		if (index)
